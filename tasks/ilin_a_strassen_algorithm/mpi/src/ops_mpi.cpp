...
void IlinAStrassenAlgorithmMPI::addMatrix(const std::vector<double> &A,
...
void IlinAStrassenAlgorithmMPI::subtractMatrix(const std::vector<double> &A,
                                               const std::vector<double> &B,
                                               std::vector<double> &C, int n) {
  for (int i = 0; i < n * n; ++i) {
    C[i] = A[i] - B[i];
...
void IlinAStrassenAlgorithmMPI::splitMatrix(const std::vector<double> &A,
...
void IlinAStrassenAlgorithmMPI::joinMatrix(std::vector<double> &A,
...
std::vector<double> IlinAStrassenAlgorithmMPI::naiveMultiplySeq(
...
std::vector<double> IlinAStrassenAlgorithmMPI::strassenSequential(
    const std::vector<double> &A, const std::vector<double> &B, int n) {
  ...
  std::vector<double> A11(half * half), A12(half * half), 
                     A21(half * half), A22(half * half);
  std::vector<double> B11(half * half), B12(half * half), 
                     B21(half * half), B22(half * half);
  ...
  std::vector<double> C11(half * half), C12(half * half),
                     C21(half * half), C22(half * half);
...
std::vector<double> IlinAStrassenAlgorithmMPI::distributedNaiveMultiply(
    const std::vector<double> &A, const std::vector<double> &B, int n) {
  ...
  MPI_Gatherv(send_data, send_count, MPI_DOUBLE,
              recv_data, recvcounts_ptr, displs_ptr, MPI_DOUBLE,
              0, MPI_COMM_WORLD);
...
std::vector<double> IlinAStrassenAlgorithmMPI::parallelStrassenRecursive(
    const std::vector<double> &A, const std::vector<double> &B, int n) {
  ...
  std::vector<double> P1_buffer, P2_buffer, P3_buffer, P4_buffer, 
                     P5_buffer, P6_buffer, P7_buffer;
  
  if (P1.empty()) P1.resize(matrix_size, 0.0);
  if (P2.empty()) P2.resize(matrix_size, 0.0);
  if (P3.empty()) P3.resize(matrix_size, 0.0);
  if (P4.empty()) P4.resize(matrix_size, 0.0);
  if (P5.empty()) P5.resize(matrix_size, 0.0);
  if (P6.empty()) P6.resize(matrix_size, 0.0);
  if (P7.empty()) P7.resize(matrix_size, 0.0);
  
  P1_buffer.resize(matrix_size * world_size_);
  MPI_Allgather(P1.data(), matrix_size, MPI_DOUBLE,
                P1_buffer.data(), matrix_size, MPI_DOUBLE, MPI_COMM_WORLD);
  
  P2_buffer.resize(matrix_size * world_size_);
  MPI_Allgather(P2.data(), matrix_size, MPI_DOUBLE,
                P2_buffer.data(), matrix_size, MPI_DOUBLE, MPI_COMM_WORLD);
  
  P3_buffer.resize(matrix_size * world_size_);
  MPI_Allgather(P3.data(), matrix_size, MPI_DOUBLE,
                P3_buffer.data(), matrix_size, MPI_DOUBLE, MPI_COMM_WORLD);
  
  P4_buffer.resize(matrix_size * world_size_);
  MPI_Allgather(P4.data(), matrix_size, MPI_DOUBLE,
                P4_buffer.data(), matrix_size, MPI_DOUBLE, MPI_COMM_WORLD);
  
  P5_buffer.resize(matrix_size * world_size_);
  MPI_Allgather(P5.data(), matrix_size, MPI_DOUBLE,
                P5_buffer.data(), matrix_size, MPI_DOUBLE, MPI_COMM_WORLD);
  
  P6_buffer.resize(matrix_size * world_size_);
  MPI_Allgather(P6.data(), matrix_size, MPI_DOUBLE,
                P6_buffer.data(), matrix_size, MPI_DOUBLE, MPI_COMM_WORLD);
  
  P7_buffer.resize(matrix_size * world_size_);
  MPI_Allgather(P7.data(), matrix_size, MPI_DOUBLE,
                P7_buffer.data(), matrix_size, MPI_DOUBLE, MPI_COMM_WORLD);
  ...
  for (int proc = 0; proc < world_size_; proc++) {
    ...
    if (P1_has_data && P1[0] == 0.0) {
      std::copy(P1_buffer.begin() + offset, 
                P1_buffer.begin() + offset + matrix_size, 
                P1.begin());
    }
    ...
    if (P2_has_data && P2[0] == 0.0) {
      std::copy(P2_buffer.begin() + offset, 
                P2_buffer.begin() + offset + matrix_size, 
                P2.begin());
    }
    ...
    if (P3_has_data && P3[0] == 0.0) {
      std::copy(P3_buffer.begin() + offset, 
                P3_buffer.begin() + offset + matrix_size, 
                P3.begin());
    }
    ...
    if (P4_has_data && P4[0] == 0.0) {
      std::copy(P4_buffer.begin() + offset, 
                P4_buffer.begin() + offset + matrix_size, 
                P4.begin());
    }
    ...
    if (P5_has_data && P5[0] == 0.0) {
      std::copy(P5_buffer.begin() + offset, 
                P5_buffer.begin() + offset + matrix_size, 
                P5.begin());
    }
    ...
    if (P6_has_data && P6[0] == 0.0) {
      std::copy(P6_buffer.begin() + offset, 
                P6_buffer.begin() + offset + matrix_size, 
                P6.begin());
    }
    ...
    if (P7_has_data && P7[0] == 0.0) {
      std::copy(P7_buffer.begin() + offset, 
                P7_buffer.begin() + offset + matrix_size, 
                P7.begin());
    }
  }
...
std::vector<double> IlinAStrassenAlgorithmMPI::parallelStrassen(
    const std::vector<double> &A, const std::vector<double> &B, int n) {
  
  return parallelStrassenRecursive(A, B, n);
}

std::vector<double> IlinAStrassenAlgorithmMPI::multiplyMatrices(
    const std::vector<double> &A, const std::vector<double> &B, int n) {
