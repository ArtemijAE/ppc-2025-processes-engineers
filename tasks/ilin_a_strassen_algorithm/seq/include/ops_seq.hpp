...
class IlinAStrassenAlgorithmSEQ : public BaseTask {
 ...
 private:
  ...
  std::vector<double> NaiveMultiply(const std::vector<double> &a, const std::vector<double> &b, int n);
  ...
  void ComputeResultSubmatrices(const std::vector<std::vector<double>> &products, std::vector<double> &c11,
                                std::vector<double> &c12, std::vector<double> &c21, std::vector<double> &c22, int half);
  ...
  int original_size_;
  int padded_size_;
  static constexpr int kThreshold_ = 64;
};
